...
지난 시간에, OSM을 올렸으니까([`Leaflet OSM 배경지도`](https://bbong95.github.io/leaflet.examples/leaflet_osm.html){:target="_blank"} <i class="fa fa-external-link"></i>) OSM 배경지도와 google map 배경지도를 사용자가 선택하여 선택된 배경지로를 표시하는 것 까지 해 보려고 한다.
