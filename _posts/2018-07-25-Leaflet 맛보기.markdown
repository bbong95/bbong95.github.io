...
- 좌표(체)계: 좌표(체)계에 대한 이해가 필요한 이유는, 각 상용지도(공개지도)마다 사용하는 좌표계가 차이가 있고, 그로 인해 위경도 값이 약간 다르기 때문이다.</br>
(다음 글들을 읽어보고, 더 깊은 내용을 공부하고 싶으면, 구글링을 해 보자.)
먼저, `좌표계에 대한 이해` http://www.biz-gis.com/index.php?mid=pds&document_srl=65754 {:target="_blank"} </br>
`한국 주요 좌표계 EPSG코드 및 PROJ4 인자 정리` http://www.osgeo.kr/17 {:target="_blank"}
